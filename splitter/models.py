...
class SplitJob(models.Model):
    ...
    should_split = models.BooleanField(
        default=True, help_text="Se deve dividir os PDFs em partes menores após compressão"
    )
    ...
    total_output_files = models.IntegerField(
        null=True, blank=True, help_text="Quantidade de PDFs gerados"
    )
    total_output_size_mb = models.FloatField(
        null=True, blank=True, help_text="Tamanho do arquivo ZIP resultante em MB"
    )
    output_zip_path = models.CharField(
        max_length=500, blank=True, help_text="Caminho do arquivo ZIP resultante"
    )
